...
MediaFileManagerController::MediaFileManagerController(
    MediaFileManager &mediaManager, std::shared_ptr<BaseView> mediaManagerView,
    std::shared_ptr<BaseView> mediaFileView,
    std::shared_ptr<MediaScannerController> c)
    : mediaManager(mediaManager), mediaFileManagerView(mediaManagerView),
      mediaFileHandlerView(mediaFileView), scannerController(c) {}

void MediaFileManagerController::addMediaFileController(
    std::string fileName,
    std::shared_ptr<MediaFileController> mediaFileController) 
{
...
std::shared_ptr<MediaFileController>
MediaFileManagerController::getMediaFileController(
...
int MediaFileManagerController::addDataFile(std::string pathName) 
{
  ...
  std::string fileName = (lastSlashPos != std::string::npos)
                             ? pathName.substr(lastSlashPos + 1)
                             : pathName;
  std::string extension =
      (lastDotPos != std::string::npos) ? pathName.substr(lastDotPos + 1) : "";
...
{
  if (mediaFileManagerView->showConfirmMenu(
...
void MediaFileManagerController::handlerMediaFileManager() 
{
  int choice;
  std::string message;
  std::string error;
  ...
  while (true) {
    ...
    try {
      ...
      {
        try {
          std::cout << "Enter your path of file to add (or press 0 to return "
                       "to the menu): ";
          std::string pathFile;
          if (std::cin.peek() == '\n') {
            std::cin.ignore();
      ...
      case SHOW_ALL_MEDIAFILE: 
      {
        ...
        if (listMediaFileController.find(filename) ==
            listMediaFileController.end()) {
          std::shared_ptr<MediaFileController> mediaFileController =
              std::make_shared<MediaFileController>(
                  mediaManager.getMediaFile(filename), mediaFileHandlerView);
          listMediaFileController[filename] = mediaFileController;
        }

...
void MediaFileManagerView::showSuccessMessage(std::string message)
{
    BaseView::showSuccessMessage(message);
}

bool MediaFileManagerView::showConfirmMenu(std::string message)
{
...
std::string MediaFileManagerView::displayAllMediaFile(MediaFileManager MediaFileManager) 
{
    ...
    if (event.is_mouse()) {
        ...
        int visible_rows = std::min(rows_per_page, (int)media_files.size() - current_page * rows_per_page);
        int exit_y_position = visible_rows + 1;  // Vị trí dòng nút Exit dựa trên số dòng hiển thị
        int y_offset = (event.mouse().y - 5) / row_height;  // Tính chỉ số dòng dựa trên tọa độ y

        if (event.mouse().motion == Mouse::Moved) {
            // Kiểm tra nếu chuột hover vào bảng
            if (y_offset >= 0 && y_offset < visible_rows) {

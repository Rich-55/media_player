...
TEST_F(PlayerControllerTest, StopPlaybackThreadRealTest) {
    // Arrange
    std::vector<std::string> files = {"song1.mp3"};
    PlayerController playerController(files);

    // Temporarily disable the mock for stopPlaybackThread in this test
    // ON_CALL(mockPlayerController, stopPlaybackThread()).WillByDefault(::testing::Invoke(&playerController, &PlayerController::stopPlaybackThread));

    // Create a flag to signal when the playback thread should stop
    std::atomic<bool> stopThreadFlag(false);

    // Create the playback thread
    std::thread playbackThread([&playerController, &stopThreadFlag]() {
        // Simulate some work done by playbackWorker
        while (!stopThreadFlag) {  // Check for stop signal
            std::this_thread::sleep_for(std::chrono::milliseconds(10));  // Simulate thread activity
        }
        // Simulate stopping playback
        playerController.stopPlaybackThread(); // Make sure it's stopping properly
    });

    // Give the thread some time to start and do some work
    std::this_thread::sleep_for(std::chrono::milliseconds(100));

    // Ensure the thread is running initially
    EXPECT_TRUE(playbackThread.joinable()) << "Playback thread should be running.";

    // Act: Signal the thread to stop and call the real method directly
    stopThreadFlag = true;  // Signal to stop the thread
    playerController.stopPlaybackThread();  // Call the real stopPlaybackThread method to handle cleanup

    // Allow some time for the thread to be properly stopped and joined
    std::this_thread::sleep_for(std::chrono::milliseconds(200));

    // Assert: The thread should no longer be joinable (i.e., it should be stopped)
    EXPECT_FALSE(playbackThread.joinable()) << "Playback thread should be stopped after calling stopPlaybackThread.";

    // Assert: Ensure the playing flag is set to false
    EXPECT_FALSE(playerController.isPlaying()) << "Expected 'playing' to be false after stopping the playback thread.";

    // Clean up: Join the thread to avoid termination issues
    if (playbackThread.joinable()) {
        playbackThread.join();
    }
}

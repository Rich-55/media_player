...
TEST(ControllerManagerTest, CheckUart_Success) {
    ...
    EXPECT_CALL(*uartManager, getPortList()).WillOnce(Return(std::vector<std::string>{"COM1", "COM2"}));
    ...
    EXPECT_CALL(*mockView, getPortAndBaudRate(testing::_, testing::_))
        .WillOnce(Return(std::make_pair("COM1", 9600)));
    EXPECT_CALL(*uartManager, checkPortConnection("COM1", 9600)).WillOnce(Return(true));
...
TEST(ControllerManagerTest, MediaFileHandler_FileRunning) {
    ...
    PlayerController::currentPlayingFile = "/music/song.mp3";
